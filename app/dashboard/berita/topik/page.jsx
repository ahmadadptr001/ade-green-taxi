...
export default function TopicsPage() {
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      const response = await addTopic(name, slug);
      setInputValue('');
      Swal.fire({
        icon: 'success',
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      const response = await removeTopic(id);
      setInputValue('');
      Swal.fire({
        icon: 'success',
        text: response.message,
        ...
        <div className="flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
          <div className="space-y-1">
            ...
            <p className="text-lg text-slate-500 font-medium">
              Kelola topik berita untuk memudahkan pencarian
            </p>
          </div>
        ...
        <div className="sticky top-4 z-10">
          <div className="flex flex-col gap-3 p-2 bg-white/80 backdrop-blur-xl border border-white/20 shadow-xl rounded-2xl md:flex-row md:items-center">
            ...
            <div className="relative flex-1 group">
              ...
              <Input
                placeholder="Cari topik atau slug..."
                    ...
                    <AlertDialog>
                      ...
                      <AlertDialogContent>
                        <AlertDialogHeader>
                          ...
                          <AlertDialogDescription>
                            Anda yakin ingin menghapus topik{' '}
                            <strong>{topic.name}</strong>? Tindakan ini tidak
                            bisa dibatalkan.
                          </AlertDialogDescription>
                        </AlertDialogHeader>
                        <AlertDialogFooter>
        ...
        {filteredTopics.length === 0 && (
          <div className="flex flex-col items-center justify-center py-20 text-center animate-in fade-in zoom-in duration-500">
            ...
            <p className="text-slate-500 max-w-xs mt-2">
              Coba kata kunci lain atau tambahkan topik baru untuk memulai.
            </p>
          </div>
        )}
      </div>
