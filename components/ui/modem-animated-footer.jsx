...
/**
 * Animated footer — centered brand, social + nav links, a giant gradient
 * wordmark behind a floating logo badge. JSX port of the 21st.dev
 * modem-animated-footer (no extra deps).
 */
export const Footer = ({
  brandName = "YourBrand",
    ...
    <section className={cn("relative mt-0 w-full overflow-hidden", className)}>
      <footer className="relative mt-20 border-t bg-background">
        <div className="relative mx-auto flex min-h-[30rem] max-w-7xl flex-col justify-between p-4 py-10 sm:min-h-[35rem] md:min-h-[40rem]">
          ...
          <div className="mt-20 flex flex-col items-center justify-center gap-2 px-4 md:mt-24 md:flex-row md:justify-between md:gap-1 md:px-0">
        ...
        {/* Bottom line */}
        <div className="absolute bottom-32 left-1/2 h-1 w-full -translate-x-1/2 bg-gradient-to-r from-transparent via-border to-transparent backdrop-blur-sm sm:bottom-34"></div>

        {/* Bottom shadow */}
        <div className="absolute bottom-28 h-24 w-full bg-gradient-to-t from-background via-background/80 to-background/40 blur-[1em]"></div>
      </footer>
    </section>
  );
};
